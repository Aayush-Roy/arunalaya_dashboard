...
function ServiceForm({ service, onSubmit, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (service) {
        const res = await axios.put(
          `https://arunalayabackendv2-1.onrender.com/api/services/${service._id}`,
          data
        );
        onSubmit(res.data);
      } else {
        const res = await axios.post(
          "https://arunalayabackendv2-1.onrender.com/api/services",
          data
        );
        onSubmit(res.data);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          required
          className="w-full bg-[#1e1e1e] border border-gray-800 rounded-lg px-4 py-3 text-white focus:border-[#f88310]"
        />
      </div>
      ...
      <div className="flex gap-4">
        <button
          type="submit"
          className="flex-1 bg-[#f88310] hover:bg-[#d97410] text-white font-semibold px-6 py-3 rounded-lg"
        >
          {service ? "Update Service" : "Create Service"}
        </button>
        ...
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 bg-gray-800 hover:bg-gray-700 text-white font-semibold px-6 py-3 rounded-lg"
          >
            Cancel
          </button>
        )}
